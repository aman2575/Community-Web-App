...
export default function Home() {
  ...
  const addLike = async(id) =>{
    ...
    post.like = posts.like +1;
    delete post.createdAt;
    delete post.updatedAt;

    const postD = await API.graphql(graphqlOperations(updatePost, { input: post}))
    const postL = {...posts}
    postL[idx] = postD.data.updatePost;
    setPosts(postL)
    console.log(postL)}
    catch(error){console.log("Error on Like")}
